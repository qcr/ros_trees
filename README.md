...
RV Trees contains a number of custom implementations & extensions of the behaviour trees defined in [py_trees](https://py-trees.readthedocs.io/en/devel/). This package gives us all of the ground-level features & functionality needed to use behaviour trees extensively in our research outcomes. The classes contained in this package are a base to help you build behaviour trees for solving tasks, but in general should not be expanded or changed. See [the README of rv_trees](https://bitbucket.org/acrv/rv_trees/src/master/) for full details & an example of how to get a robot solving a task using trees.
...
- **pre**: other leaves have run before this leaf & stored data in the [py_trees blackboard](https://py-trees.readthedocs.io/en/devel/blackboards.html) (a centralised key-value dict). There is also the special method `get_last_value()` from `rv_trees.data_management` that will 'magically' get the last saved result from a previous leaf. 
- **load data**: the leaf will attempt to load any data it may need by calling `load_fn`. If you don't provide a `load_fn` argument there is a default behaviour that should work for most cases. The default `load_fn` (see `Leaf._default_load_fn()`) will load from the blackboard key `load_key` if provided, otherwise it uses the last saved value through `get_last_value()`.
...
- **evaluate the result**: `eval_fn` is called to determine whether the leaf's process was a success or failure. If not `eval_fn` is provided, the default will return the first bool in `self.result` if it is a list, otherwise the Boolean evaluation of `self.result`.
- **leaving**: the leaf is done; no more actions should be performed by the leaf from this point on.
- **saving the result**: if the `save` flag is set, the leaf will attempt to save `self.result` according to `save_fn`. The default `save_fn` should be fine for most cases; it saves the result to key `save_key` if it is set, otherwise the result is saved such that it is available to the next leaf in the tree with `get_last_value()`.
- **post**: the tree has decided the leaf is done, if there is any steps your leave needs to perform to stop its action you should overload `_extra_terminate()`. For example, an `ActionLeaf` sends the pre-empt signal by overloading `extra_terminate()`.

That is a general overview of the lifecycle of a leaf in `rv_trees`. We have covered what you will need to know about leaves for 99% of cases, but there are also other methods you can override to control other parts of the process. See the class implementation `rv_trees.leaves.Leaf` for full details.
...
ROS can be a bit torturous when it comes to passing data between processes For example, if you have a service response `MyServiceResponse` with fields `PoseStamped` & an Action Server with goal `MyActionGoal` with exactly the same fields you have to manually do that conversion for every possible type of input data even though have exactly the same fields. This can become extremely tedious in the scope of a tree, especially given that a leaf should be written in a way that it is as input agnostic as possible!
...
- To make sure people use your leaves (once your sure it is usable), include it in the appropriate part of `rv_trees.common_leaves.*`
- Implement your leaves by extending an existing class (e.g. `classdef MyLeaf(rv_trees.leaves.Leaf): ...`)
- The less code in your leaf implementation, generally the better. Don't be afraid of having a leaf class that is literally just setting a default parameter (that's what most `ActionLeaf`s & `ServiceLeaf`s end up being).
